...
"""
    ...
    def live_tick(self):
        ...
        # 0.5.9.15: CPU/GPU-Entlastung. Bis zu 13 Overlay-Fenster sind einzeln
        ...
        # im Cockpit/auf der Strecke). Im Menue/Stand/Box (is_hard_garage_sample)
        # aendern sich die angezeigten Werte kaum -> dort reicht 2 Hz, das reduziert
        # die DWM-Kompositierlast dort um 80% und entlastet spuerbar auch LMUs
        # eigenes Menue. Rein additiv: refresh_overlay()/paintEvent-Logik unveraendert.
        self._overlay_tick_n = getattr(self, "_overlay_tick_n", 0) + 1
        try:
            _driving = not self.is_hard_garage_sample(s)
        except Exception:
            _driving = True
        if _driving or self._overlay_tick_n % 5 == 0:
            self.refresh_overlay()
