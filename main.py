...
class LMUReader:
    ...
    def read_sample(self) -> Sample:
        ...
        try:
            ...
            _gap_behind = 0.0
        # 0.4.9.9: Gegnerpositionen fuer den Spotter (Weltkoordinaten, Bodenebene)
        _opps = []
        try:
            ...
            for _i in range(min(int(scoring_info.mNumVehicles), LMUConstants.MAX_MAPPED_VEHICLES)):
                ...
                if int(_v.mID) == _ego_id:
                    continue
                _p = _v.mPos
                _opps.append((int(_v.mID), fnum(_p.x), fnum(_p.z), bool(_v.mInPits)))
        except Exception:
            _opps = []
        def w_brake_temp(i):
    ...
    def __init__(self):
        ...
        if jcm_path.exists():
            self.jcm_logo.setPixmap(QPixmap(str(jcm_path)).scaled(
                150, 68, Qt.KeepAspectRatio, Qt.SmoothTransformation))
