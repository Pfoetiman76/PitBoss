...
class TireMiniWidget(OverlayStyleMixin, QWidget):
    """Skalierbare Reifenanzeige (VR-tauglich). 0.5.9.1: EIN vertikaler Balken,
    Balkenhoehe = Rest-Verschleiss, Ampel (>=75 gruen / >=40 gelb / sonst rot),
    Carcass-Temp neben dem Balken. Temp-Zonen/Kopf-Rest/Trend entfernt.
    Flat/Lock-Warnung nur wenn warn_enabled."""
    def __init__(self, name: str, value_side: str = "right"):
        ...
        self.setToolTip("Reifen: Balkenhoehe = Rest-Verschleiss (voll/gruen = 100 %, "
                        "gelb ab <75 %, rot ab <40 %). Carc = Karkass-Temp neben dem Balken. "
    ...
    def paintEvent(self, event):
        ...
        # Titel
        F(min(h * 0.11, 16), True); p.setPen(self._fg())
        p.drawText(QRectF(0, h * 0.01, w, h * 0.15), Qt.AlignCenter, self.name)

        # Druck (Kopfzeile, vollbreit zentriert). 0.5.9.1: Verschleiss-Kopfzahl entfernt.
        F(min(h * 0.078, 12), False); p.setPen(self._muted())
        p.drawText(QRectF(0, h * 0.17, w, h * 0.11), Qt.AlignCenter,
                   f"{self.pressure_kpa / 100.0:.2f} bar")

        # 0.5.9.1: EIN vertikaler Balken, Hoehe = Rest-Verschleiss.
        #   voll & gruen bei 100 %, ab <75 % gelb, ab <40 % rot.
        #   LMU liefert mWear nur als EINEN Wert je Rad (keine Zonen).
        bar_x = w * 0.28; bar_w = w * 0.26
        bar_top = h * 0.33; bar_bot = h * 0.93; bar_h = bar_bot - bar_top
        col = self._bar_color(self.wear)
        p.setPen(QPen(self._border(), 1)); p.setBrush(QBrush(self._trackcol()))
        p.drawRoundedRect(QRectF(bar_x, bar_top, bar_w, bar_h), 4, 4)
        fillh = bar_h * max(0.0, min(1.0, self.wear / 100.0))
        if fillh > 1.0:
            p.setPen(Qt.NoPen); p.setBrush(QBrush(col))
            p.drawRoundedRect(QRectF(bar_x, bar_bot - fillh, bar_w, fillh), 4, 4)

        # Rest-% als Zahl mittig auf dem Balken (Ampelfarbe)
        F(min(bar_w * 0.42, 14), True); p.setPen(col)
        p.drawText(QRectF(bar_x - w * 0.03, bar_top + bar_h * 0.5 - h * 0.05,
                          bar_w + w * 0.06, h * 0.1), Qt.AlignCenter, f"{self.wear:.0f}%")

        # 0.5.9.1: Carcass-Temp NEBEN dem Balken (Temp-Zonen + Trend entfernt).
        cx = bar_x + bar_w + w * 0.05
        cw = w - cx - w * 0.03
        F(min(h * 0.062, 10), False); p.setPen(self._muted())
        p.drawText(QRectF(cx, bar_top + bar_h * 0.30, cw, h * 0.09),
                   Qt.AlignHCenter | Qt.AlignBottom, "Carc")
        F(min(h * 0.135, 20), True); p.setPen(self._fg())
        p.drawText(QRectF(cx, bar_top + bar_h * 0.39, cw, h * 0.16),
                   Qt.AlignHCenter | Qt.AlignTop, f"{self.carcass:.0f}\u00b0")

        # Flat/Lock-Warnung (opt-in) rahmt jetzt den Verschleiss-Balken
        ...
        if self.warn_enabled and fs != "OK":
            ...
            p.drawRoundedRect(QRectF(bar_x - 4, bar_top - 4, bar_w + 8, bar_h + 8), 6, 6)
            ...
            p.drawText(QRectF(bar_x - w * 0.15, bar_top - h * 0.10, bar_w + w * 0.3, h * 0.09), Qt.AlignCenter, label)
        p.end()
