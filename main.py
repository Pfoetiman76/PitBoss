...
class SetupCoachWindow(QWidget):
    ...
    def refresh(self):
        ...
        if self._file_garage:
            ...
            if diag and diag.get("type") == "svm":
                src = (f"Quelle: {fname} (LMU-.svm, Momentaufnahme) — "
                       f"{diag.get('lines', 0)} Parameter gelesen, "
                       f"{diag.get('mapped', 0)} für Coach-Hinweise erkannt")
            else:
                src = f"Quelle: {fname} (getPlayerGarage-Datei, Momentaufnahme)"
        elif garage:
        ...
        if self._file_garage and diag and diag.get("type") == "svm":
    ...
    def _svm_raw_block(self, diag):
        ...
        if not raw:
            return []
        out = ["", "[Alle Werte aus der .svm-Datei]"]
        last_sec = object()
        ...
        out.append("Fehlt was Wichtiges, schick mir diese .svm — dann ziehe ich das Mapping exakt.")
        return out
    ...
    def load_from_file(self):
        """0.5.9.8: laedt entweder eine in LMU manuell gespeicherte .svm
        (rF2/LMU-Setupformat) ODER einen getPlayerGarage-JSON/-txt-Dump.
        Format wird automatisch erkannt."""
        from PySide6.QtWidgets import QFileDialog
        import json
        path, _ = QFileDialog.getOpenFileName(
            self, "Setup laden (LMU .svm oder getPlayerGarage .json/.txt)", "",
        ...
        g = None
        diag = None
        # 1) getPlayerGarage-JSON zuerst versuchen
        if text.lstrip()[:1] in ("{", "["):
            try:
                ...
                g = None
        # 2) sonst als .svm/INI-Setup parsen
        if not g:
            try:
                g, diag = parse_svm_setup(text)
            except Exception as e:
                self.src_label.setText(f"Datei-Fehler (.svm): {e}")
                return
        if not g:
            self.src_label.setText(
                "Datei-Fehler: keine Setup-Parameter erkannt "
                "(weder getPlayerGarage-JSON noch lesbares .svm-Setup).")
            return
        self._file_garage = g
