...
SPOTTER_CONFIG_PATH = EXPORT_DIR / "spotter.json"  # 0.4.9.9: Spotter (Flip)
...
class TireMiniWidget(OverlayStyleMixin, QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Titel
        F(min(h * 0.095, 15), True); p.setPen(self._fg())
        p.drawText(QRectF(0, h * 0.005, w, h * 0.13), Qt.AlignCenter, self.name)
        # Druck links, Reifen-Rest rechts (Kopfzahl, Farbe nach Rest-Level)
        F(min(h * 0.066, 10), False); p.setPen(self._muted())
        p.drawText(QRectF(0, h * 0.145, w * 0.5, h * 0.10), Qt.AlignCenter, f"{self.pressure_kpa / 100.0:.2f} bar")
        F(min(h * 0.09, 14), True); p.setPen(self._wear_color(self.wear))
        p.drawText(QRectF(w * 0.5, h * 0.125, w * 0.5, h * 0.10), Qt.AlignCenter, f"{self.wear:.0f}%")
        F(min(h * 0.043, 7), False); p.setPen(self._muted())
        p.drawText(QRectF(w * 0.5, h * 0.223, w * 0.5, h * 0.05), Qt.AlignCenter, "Rest")

        # 0.5.8.0: prominenter Verschleiss-Balken (Rest %), vollbreit. LMU liefert
        # mWear nur als EINEN Wert je Rad (keine Zonen), deshalb ein Balken statt
        # drei. Fuellung = Rest, Farbe gruen (voll) -> rot (abgefahren).
        wb_x = w * 0.10; wb_w = w * 0.80
        wb_top = h * 0.285; wb_h = max(6.0, h * 0.075)
        p.setPen(QPen(self._border(), 1)); p.setBrush(QBrush(self._trackcol()))
        p.drawRoundedRect(QRectF(wb_x, wb_top, wb_w, wb_h), 3, 3)
        fillw = wb_w * max(0.0, min(1.0, self.wear / 100.0))
        if fillw > 1.0:
            p.setPen(Qt.NoPen); p.setBrush(QBrush(self._wear_color(self.wear)))
            p.drawRoundedRect(QRectF(wb_x, wb_top, fillw, wb_h), 3, 3)
        F(min(wb_h * 0.66, 9), True); p.setPen(self._fg())
        p.drawText(QRectF(wb_x, wb_top, wb_w, wb_h), Qt.AlignCenter, "VERSCHLEISS")
        ...
        x0 = (w - group_w) / 2.0
        bar_top = h * 0.42; bar_bot = h * 0.66; bar_h = bar_bot - bar_top
        temp_y = h * 0.68

        for i, t in enumerate(temps):
            x = x0 + i * (bar_w + bar_gap)
            tpct = self._temp_pct(t)
            cell = QRectF(x - bar_gap / 2.0, 0, bar_w + bar_gap, h)
            # Balken: Hoehe & Farbe = Zonentemperatur (echte Pro-Zone-Daten)
            p.setPen(QPen(self._border(), 1)); p.setBrush(QBrush(self._trackcol()))
            p.drawRoundedRect(QRectF(x, bar_top, bar_w, bar_h), 4, 4)
            fillh = bar_h * max(0.06, min(1.0, tpct / 100.0))
            p.setPen(Qt.NoPen); p.setBrush(QBrush(temp_color_for_tire(t)))
            p.drawRoundedRect(QRectF(x, bar_bot - fillh, bar_w, fillh), 4, 4)
            # Temperatur in C unter dem Balken
            F(min(h * 0.068, 10), True); p.setPen(self._fg())
            p.drawText(QRectF(cell.x(), temp_y, cell.width(), h * 0.085), Qt.AlignCenter, f"{t:.0f}C")
            F(min(h * 0.05, 8), False); p.setPen(self._muted())
            p.drawText(QRectF(cell.x(), temp_y + h * 0.082, cell.width(), h * 0.06), Qt.AlignCenter, labels[i])

        # Carcass-Temp + Verschleiss-Trend im Stint
        tr = self.wear_trend
        if tr <= -0.5:
            arrow = "\u25bc" + f"{abs(tr):.0f}"
        elif tr >= 0.5:
            ...
            arrow = "\u25ac"
        F(min(h * 0.062, 10), False); p.setPen(self._muted())
        p.drawText(QRectF(0, h * 0.865, w, h * 0.125), Qt.AlignCenter,
                   f"Carc {self.carcass:.0f}\u00b0   Trend {arrow}")
        ...
        if self.warn_enabled and fs != "OK":
            ...
            p.drawRoundedRect(QRectF(x0 - 4, bar_top - 4, group_w + 8, bar_h + 8), 6, 6)
            ...
            p.drawText(QRectF(x0 - 4, bar_top - h * 0.11, group_w + 8, h * 0.09), Qt.AlignCenter, label)
        p.end()
...
class SetupCoachWindow(QWidget):
    ...
    def _garage(self):
        if self._file_garage:
            return self._file_garage
        rest = getattr(self.main, "rest", None)
        return dict(rest.garage) if (rest is not None and rest.has_garage()) else {}

    def refresh(self):
        garage = self._garage()
        if self._file_garage:
            src = "Quelle: Datei (Momentaufnahme)"
        elif garage:
            src = "Quelle: LMU-API (live, getPlayerGarage)"
        else:
            src = "Quelle: — kein Setup. LMU offen lassen und 'Aktualisieren', oder 'Setup aus Datei laden'."
        ...
        self.overview.setPlainText("\n".join(SetupAdvisor.overview_lines(garage)))
    ...
    def load_from_file(self):
        ...
        import json
        path, _ = QFileDialog.getOpenFileName(self, "getPlayerGarage-Datei", "",
                                              "Setup/JSON (*.txt *.json);;Alle Dateien (*.*)")
        if not path:
            return
        try:
            data = json.loads(open(path, encoding="utf-8", errors="replace").read())
            g = {k: v for k, v in data.items() if isinstance(v, dict) and "value" in v}
            if not g:
                raise ValueError("keine Setup-Parameter erkannt")
            self._file_garage = g
        except Exception as e:
            self.src_label.setText(f"Datei-Fehler: {e}")
            return
        self.refresh()
...
class LmuRestClient:
    ...
    GARAGE_PATH = "/rest/garage/getPlayerGarage"  # 0.5.9.0: komplettes aktuelles Setup
...
class Spotter(QObject):
    """0.4.9.9 Spotter: sagt Autos daneben an - "Auto links/rechts", "Drei
    nebeneinander", "frei". Gegnerpositionen aus dem Scoring (s.opponents),
    eigene Blickrichtung per Finite-Differenz der eigenen Position (umgeht die
    unsichere mOri-Konvention). Links/Rechts per Menue "Seiten spiegeln"
    kalibrierbar. Vorsynthetisierte Piper-Clips (latenzarm). Opt-in.
    """
    ...
    LAT_CLEAR_MAX = 9.5
    MIN_GAP_S = 0.55
    STILL_REMINDER_S = 3.0
    SPEED_GATE_MS = 8.0

    CLIPS = {"L": "Auto links", "R": "Auto rechts", "W": "Drei nebeneinander",
             "C": "frei", "CL": "frei links", "CR": "frei rechts", "S": "steht noch"}
    # 0.5.8.0: feste Flaggen-Clips (vorsynthetisiert, latenzarm)
    FLAG_CLIPS = {"YEL": "Gelbe Flagge", "YELG": "Gr\u00fcn, freie Fahrt",
                  "BLU": "Blaue Flagge, vorbeilassen"}
    RACE_CD_S = 45.0  # 0.5.8.0: Cooldown der Positions-/Gap-Ansage
    ...
    def __init__(self, main):
        ...
        self.enabled = False
        self.flip = False
        self.say_race = False    # 0.5.8.0: Position & Abstaende ansagen
        self.say_flags = False   # 0.5.8.0: Flaggen (Gelb/Blau) ansagen
        self._load()
        self._reset_state()
    ...
    def _reset_state(self):
        ...
        self._left = False
        self._right = False
        self._since_call = 999.0
        self._left_hold = 0.0
        self._right_hold = 0.0
        self._t = None
        self._flag_state = None   # 0.5.8.0: zuletzt gesehene Flagge
        self._race_cd = 0.0       # 0.5.8.0: Cooldown Positions-/Gap-Ansage
        self._last_place = None

    def _load(self):
        try:
            import json
            with open(SPOTTER_CONFIG_PATH, "r", encoding="utf-8") as f:
                d = json.load(f)
            self.flip = bool(d.get("flip", False))
            self.say_race = bool(d.get("say_race", False))
            self.say_flags = bool(d.get("say_flags", False))
        except Exception:
            self.flip = False
    ...
    def _save(self):
        try:
            import json
            EXPORT_DIR.mkdir(parents=True, exist_ok=True)
            with open(SPOTTER_CONFIG_PATH, "w", encoding="utf-8") as f:
                json.dump({"flip": self.flip, "say_race": self.say_race,
                           "say_flags": self.say_flags}, f, indent=2)
        except Exception:
            pass
    ...
    def set_say_race(self, on):
        self.say_race = bool(on)
        self._race_cd = 0.0
        self._save()

    def set_say_flags(self, on):
        self.say_flags = bool(on)
        self._flag_state = None
        self._save()
        if self.say_flags:
            try:
                self.main.tts.prewarm_clips(self.FLAG_CLIPS)
            except Exception:
                pass
    ...
    def _zone(self, along, side, prev):
        lat = abs(side)
        side_key = "right" if side > 0 else "left"
        if prev is None:
            beside = (self.ENTER_BACK <= along <= self.ENTER_FRONT) and \
                     (self.LAT_MIN <= lat <= self.LAT_ENTER_MAX)
            return side_key if beside else None
        still = (self.CLEAR_BACK <= along <= self.CLEAR_FRONT) and (lat <= self.LAT_CLEAR_MAX)
        if not still:
            return None
        if lat >= self.LAT_MIN and side_key != prev:
            return side_key
        return prev

    def _emit(self, now_left, now_right):
        calls = []
        both_new = (now_left and now_right) and not (self._left and self._right)
        if both_new:
            calls.append("W")
        else:
            if now_left and not self._left:
                calls.append("L")
            if now_right and not self._right:
                calls.append("R")
        if self._left and not now_left:
            calls.append("CR" if now_right else "C")
        if self._right and not now_right:
            calls.append("CL" if now_left else "C")
        if self.STILL_REMINDER_S > 0 and not calls:
            if now_left and self._left and self._left_hold >= self.STILL_REMINDER_S:
                calls.append("S"); self._left_hold = 0.0
            elif now_right and self._right and self._right_hold >= self.STILL_REMINDER_S:
                calls.append("S"); self._right_hold = 0.0
        if not calls:
            return
        clear = next((k for k in calls if k in ("C", "CL", "CR")), None)
        if clear is not None:
            self._say(clear)
            self._since_call = 0.0
        elif self._since_call >= self.MIN_GAP_S:
            self._say(calls[0])
            self._since_call = 0.0
    ...
    def _say_flag(self, key):
        try:
            self.main.tts.play_clip(key, self.FLAG_CLIPS.get(key, ""))
        except Exception:
            pass

    def _update_flags(self, s):
        """0.5.8.0: Gelb-/Blau-Flagge reaktiv ansagen (nur bei Wechsel),
        Aufhebung nach Gelb als 'gruen frei'. Nutzt die Flag-Overlay-Logik."""
        try:
            flag, _lbl = FlagOverlayWindow._decide(s)
        except Exception:
            return
        prev = self._flag_state
        if flag == prev:
            return
        if flag == "yellow":
            self._say_flag("YEL")
            ...
            self._say_flag("BLU")
        elif prev == "yellow" and flag in ("green", "none"):
            self._say_flag("YELG")
        self._flag_state = flag

    @staticmethod
    def _fmt_gap(g):
        return f"{g:.1f}".replace(".", ",") + " Sekunden"

    def _update_race(self, s, dt):
        """0.5.8.0: aktuelle Position + Abstand vorn/hinten ansagen. Nur bei
        zuegiger Fahrt, im Renn-/FCY-Kontext, wenn kein Auto daneben ist
        (Seiten-Rufe haben Vorrang). Cooldown RACE_CD_S. Live-Synthese."""
        self._race_cd = max(0.0, self._race_cd - dt)
        if self._left or self._right:
            return
        if float(getattr(s, "speed_kmh", 0.0)) < 80.0:
            return
        if int(getattr(s, "game_phase", 0) or 0) not in (5, 6):
            return
        place = int(getattr(s, "place", 0) or 0)
        if place <= 0 or self._race_cd > 0.0:
            return
        parts = [f"Position {place}"]
        ga = float(getattr(s, "gap_ahead_s", 0.0) or 0.0)
        gb = float(getattr(s, "gap_behind_s", 0.0) or 0.0)
        if 0.0 < ga < 30.0:
            parts.append(f"vorne {self._fmt_gap(ga)}")
        if 0.0 < gb < 30.0:
            parts.append(f"hinten {self._fmt_gap(gb)}")
        try:
            self.main.tts.speak(", ".join(parts) + ".", interrupt=False)
        except Exception:
            pass
        self._race_cd = self.RACE_CD_S
        self._last_place = place

    def update(self, s):
        if not (self.enabled or self.say_race or self.say_flags):
            return
        import time
        ...
        in_pits = bool(getattr(s, "in_pits", False)) or int(getattr(s, "pit_state", 0)) > 0
        speed_ms = float(getattr(s, "speed_kmh", 0.0)) / 3.6

        # 0.5.8.0: Flaggen immer (auch stehend), Position/Gap nicht in der Box
        if self.say_flags:
            self._update_flags(s)
        if self.say_race and not in_pits:
            self._update_race(s, dt)

        if not self.enabled:
            return

        hd = self._heading(float(s.pos_x), float(s.pos_z))

        if in_pits or hd is None or speed_ms < self.SPEED_GATE_MS:
            self._car_side.clear()
            self._left = self._right = False
            self._left_hold = self._right_hold = 0.0
            return

        ex, ez = float(s.pos_x), float(s.pos_z)
        fx, fz, rx, rz = hd
        seen = set()
        now_left = now_right = False

        for opp in getattr(s, "opponents", ()):
            try:
                cid = int(opp[0]); ox = float(opp[1]); oz = float(opp[2]); opits = bool(opp[3])
            except Exception:
                continue
            if opits:
                continue
            ...
            along = dx * fx + dz * fz
            side = dx * rx + dz * rz
            prev = self._car_side.get(cid)
            zone = self._zone(along, side, prev)
            self._car_side[cid] = zone
            if zone == "left":
                now_left = True
            elif zone == "right":
                now_right = True

        for cid in [k for k in self._car_side if k not in seen]:
            del self._car_side[cid]

        self._left_hold = self._left_hold + dt if now_left else 0.0
        self._right_hold = self._right_hold + dt if now_right else 0.0

        self._emit(now_left, now_right)
        self._left, self._right = now_left, now_right
...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.spotter_enabled = False  # 0.4.9.9: Spotter (opt-in)
        self.spotter = Spotter(self)  # 0.4.9.9
        ...
        _coach.addSeparator()
        self.act_spotter = _coach.addAction("Spotter: Auto links/rechts (Ansagen)")
        self.act_spotter.setCheckable(True)
        self.act_spotter.setChecked(self.spotter_enabled)
        self.act_spotter.toggled.connect(self.set_spotter_enabled)
        self.act_spotter_flip = _coach.addAction("Spotter: Seiten spiegeln")
        self.act_spotter_flip.setCheckable(True)
        self.act_spotter_flip.setChecked(self.spotter.flip)
        self.act_spotter_flip.toggled.connect(self.set_spotter_flip)
        self.act_spotter_race = _coach.addAction("Spotter: Position & Abst\u00e4nde")  # 0.5.8.0
        self.act_spotter_race.setCheckable(True)
        self.act_spotter_race.setChecked(self.spotter.say_race)
        self.act_spotter_race.toggled.connect(self.set_spotter_race)
        self.act_spotter_flags = _coach.addAction("Spotter: Flaggen (Gelb/Blau)")  # 0.5.8.0
        self.act_spotter_flags.setCheckable(True)
        self.act_spotter_flags.setChecked(self.spotter.say_flags)
        self.act_spotter_flags.toggled.connect(self.set_spotter_flags)
        _coach.addSeparator()
...
"""
    ...
    def open_setup_coach_window(self):  # 0.5.9.0
        ...
        self.setup_coach_window.refresh()
    ...
    def refresh_overlay(self):
        ...
        for _a in ("weather_overlay_window", "brake_overlay_window",
                   ...
                   "flag_overlay_window", "lapgraph_overlay_window", "overtake_overlay_window"):  # 0.4.9.0/0.4.9.1/0.5.1.0
    ...
    def set_spotter_enabled(self, on):
        self.spotter_enabled = bool(on)
        self.spotter.set_enabled(self.spotter_enabled)
        if self.spotter_enabled and not self.tts.is_available():
            self.log.append("Spotter an, aber keine Sprachausgabe - Piper/SAPI pruefen.")
        self.status.setText("Spotter AN - Ansagen fuer Autos daneben."
                            if self.spotter_enabled else "Spotter AUS.")

    def set_spotter_flip(self, on):
        self.spotter.set_flip(bool(on))
        self.status.setText("Spotter: Seiten " + ("gespiegelt." if on else "normal."))

    def set_spotter_race(self, on):  # 0.5.8.0
        self.spotter.set_say_race(bool(on))
        self.status.setText("Spotter: Position & Abstaende " + ("AN." if on else "AUS."))

    def set_spotter_flags(self, on):  # 0.5.8.0
        self.spotter.set_say_flags(bool(on))
        if on and not self.tts.is_available():
            self.log.append("Flaggen-Ansage an, aber keine Sprachausgabe - Piper/SAPI pruefen.")
        self.status.setText("Spotter: Flaggen (Gelb/Blau) " + ("AN." if on else "AUS."))
