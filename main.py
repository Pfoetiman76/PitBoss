...
class TireMiniWidget(OverlayStyleMixin, QWidget):
    ...
    def paintEvent(self, event):
        ...
        F(min(h * 0.068, 10), False); p.setPen(self._muted())
        p.drawText(QRectF(0, h * 0.155, w, h * 0.10), Qt.AlignCenter, f"{self.pressure_kpa / 100.0:.2f} bar")

        temps = [self.out_t, self.mid_t, self.in_t]
        wears = [self.wear_out, self.wear_mid, self.wear_in]  # 0.5.5.1: %-Reihe = Verschleiss
        labels = ["A", "M", "I"]
        group_w = w * 0.70
        bar_gap = group_w * 0.12
        bar_w = (group_w - 2 * bar_gap) / 3.0
        x0 = (w - group_w) / 2.0
        bar_top = h * 0.35; bar_bot = h * 0.66; bar_h = bar_bot - bar_top
        pct_y = h * 0.265; temp_y = h * 0.68

        for i, t in enumerate(temps):
            x = x0 + i * (bar_w + bar_gap)
            pct = wears[i]  # 0.5.5.1: Verschleiss pro Zone (wie LMU-MFD)
            cell = QRectF(x - bar_gap / 2.0, 0, bar_w + bar_gap, h)
            # %-Wert oben
            F(min(h * 0.072, 11), True); p.setPen(self._fg())
            p.drawText(QRectF(cell.x(), pct_y, cell.width(), h * 0.085), Qt.AlignCenter, f"{pct:.0f}%")
            # Balkenrahmen + Fuellung (Hoehe & Farbe = Verschleiss)
            p.setPen(QPen(self._border(), 1)); p.setBrush(QBrush(self._trackcol()))
            p.drawRoundedRect(QRectF(x, bar_top, bar_w, bar_h), 4, 4)
            fillh = bar_h * max(0.06, min(1.0, pct / 100.0))
            p.setPen(Qt.NoPen); p.setBrush(QBrush(self._wear_color(pct)))
            p.drawRoundedRect(QRectF(x, bar_bot - fillh, bar_w, fillh), 4, 4)
            # Temperatur in C unten
        ...
        p.drawText(QRectF(0, h * 0.865, w, h * 0.125), Qt.AlignCenter,
                   f"Carc {self.carcass:.0f}\u00b0   Rest {self.wear:.0f}% {arrow}")
