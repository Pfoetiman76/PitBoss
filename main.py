...
class TireMiniWidget(OverlayStyleMixin, QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i in range(3):
            ...
            # %-Wert oben (Ampelfarbe)
            F(min(cellw * 0.40, 13), True); p.setPen(wc)
            p.drawText(QRectF(cellx, pct_top, cellw, h * 0.115), Qt.AlignCenter, f"{wv:.0f}%")
            ...
            # Zonentemperatur unter dem Balken (temperaturfarbig)
            F(min(cellw * 0.36, 12), True); p.setPen(temp_color_for_tire(zt[i]))
            p.drawText(QRectF(cellx, temp_top, cellw, h * 0.12), Qt.AlignCenter, f"{zt[i]:.0f}C")
