...
class TireMiniWidget(OverlayStyleMixin, QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Kopfzeile: Druck (bar) links, Carcass-Temp rechts.
        F(min(h * 0.072, 11), False); p.setPen(self._muted())
        p.drawText(QRectF(w * 0.03, h * 0.16, w * 0.47, h * 0.10),
                   Qt.AlignVCenter | Qt.AlignLeft, f"{self.pressure_kpa / 100.0:.2f} bar")
        p.drawText(QRectF(w * 0.50, h * 0.16, w * 0.47, h * 0.10),
                   Qt.AlignVCenter | Qt.AlignRight, f"Carc {self.carcass:.0f}\u00b0")

        # 0.5.9.4: drei Balken je Zone (A/M/I = aussen/mitte/innen).
        #   Balkenhoehe + %-Wert darueber = Verschleiss der Zone, Ampel
        #   (>=75 gruen / >=40 gelb / sonst rot). Temperatur der Zone darunter.
        #   HINWEIS: LMU-Shared-Memory hat mWear nur EINMAL pro Rad -> ohne
        #   Pro-Zone-Quelle zeigen alle drei denselben Verschleiss (wear_zones-
        #   Fallback). Sobald MFD/tireinfo pro Zone liefert, differenziert es sich.
        ...
        pct_top = h * 0.28
        bar_top = h * 0.38; bar_bot = h * 0.72; bar_h = bar_bot - bar_top
        temp_top = h * 0.75
        lbl_top = h * 0.865

        for i in range(3):
            bx = x0 + i * (b_w + gap)
            cellx = bx - gap / 2.0; cellw = b_w + gap
            wv = max(0.0, min(100.0, zw[i]))
            wc = self._bar_color(wv)
            # %-Wert ueber dem Balken (Ampelfarbe)
            F(min(b_w * 0.44, 12), True); p.setPen(wc)
            p.drawText(QRectF(cellx, pct_top, cellw, h * 0.085), Qt.AlignCenter, f"{wv:.0f}%")
            ...
            # Zonentemperatur unter dem Balken (temperaturfarbig) + Label
            F(min(h * 0.075, 11), True); p.setPen(temp_color_for_tire(zt[i]))
            p.drawText(QRectF(cellx, temp_top, cellw, h * 0.10), Qt.AlignCenter, f"{zt[i]:.0f}C")
            F(min(h * 0.05, 8), False); p.setPen(self._muted())
            p.drawText(QRectF(cellx, lbl_top, cellw, h * 0.07), Qt.AlignCenter, zl[i])
        ...
        if self.warn_enabled and fs != "OK":
            ...
            p.drawText(QRectF(x0 - 5, bar_top - h * 0.095, group_w + 10, h * 0.085), Qt.AlignCenter, label)
        p.end()
