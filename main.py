...
def _lmu_settings_dirs():
    """Gaengige LMU-Setup-Ordner (…\\UserData\\player\\Settings) auf allen Laufwerken.
    LMU/rF2 speichert manuelle Setups (.svm) dort je Auto/Strecke."""
    import string
    subs = [
    ...
    for dr in drives:
        for sub in subs:
            s = os.path.join(dr, sub, "UserData", "player", "Settings")
            if os.path.isdir(s):
                out.append(s)
    return out


def find_newest_svm(extra_dir=None):
...
class SetupCoachWindow(QWidget):
    ...
    def __init__(self, main):
        ...
        self.btn_dir.setToolTip("Ordner mit deinen .svm-Setups (…\\UserData\\player\\Settings). "
                                "Wird gemerkt; die NEUESTE .svm wird beim Öffnen automatisch geladen.")
    ...
    def _auto_load(self):
        """Laedt automatisch die NEUESTE .svm aus dem (gemerkten oder auto-gefundenen)
        LMU-Setup-Ordner; Fallback = zuletzt manuell geladene Datei."""
        newest = find_newest_svm(self._setups_dir)
    ...
    def refresh(self):
        ...
        if live:
            ...
            if diag and diag.get("type") == "svm":
                ...
                src = f"Quelle: {fname} ({tag} geladen)"
        else:
            garage = {}
            ...
            src = ("Quelle: — kein Setup gefunden. LMU offen lassen und 'Aktualisieren', "
                   "'Setup aus Datei laden', oder Ordner setzen." + hint)
