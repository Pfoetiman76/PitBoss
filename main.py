...
class TireMiniWidget(OverlayStyleMixin, QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.drawText(QRectF(w * 0.5, h * 0.125, w * 0.5, h * 0.12), Qt.AlignCenter, f"{self.wear:.0f}%")
        F(min(h * 0.045, 7), False); p.setPen(self._muted())
        p.drawText(QRectF(w * 0.5, h * 0.238, w * 0.5, h * 0.055), Qt.AlignCenter, "Rest")
        ...
        x0 = (w - group_w) / 2.0
        bar_top = h * 0.36; bar_bot = h * 0.66; bar_h = bar_bot - bar_top
        temp_y = h * 0.68
...
class SettingsWindow(QWidget):
    ...
    def __init__(self, main):
        super().__init__()
        self.main = main
        self.setWindowTitle("Einstellungen – LMU Consistency Coach")
        self.resize(430, 300)
    ...
    def _load(self):
        try:
            ...
            with open(SPOTTER_CONFIG_PATH, "r", encoding="utf-8") as f:
                self.flip = bool(json.load(f).get("flip", False))
        except Exception:
            self.flip = False
    ...
    def _save(self):
        try:
            ...
            with open(SPOTTER_CONFIG_PATH, "w", encoding="utf-8") as f:
                json.dump({"flip": self.flip}, f, indent=2)
        except Exception:
            pass
...
class Main(QMainWindow):
    def __init__(self):
        ...
        _coach.addSeparator()
        _lead = _coach.addMenu("Ansage-Vorlauf")  # 0.4.9.8
        for _k, _lbl in (("brake", "Bremsen …"), ("turnin", "Einlenken …"), ("gas", "Gas …")):
            _lead.addAction(_lbl, lambda _c=False, k=_k: self.set_coach_lead(k))
