...
`GITHUB_REPO` in `updater.py` ist bereits auf `Pfoetiman76/pitboss` gesetzt.
...
git init
git add -A
git commit -m "Initial commit: PitBoss 0.4.7.5"
git branch -M main
# Repo bei GitHub anlegen (Web) ODER mit GitHub CLI:
gh repo create pitboss --public --source=. --remote=origin --push
# ohne gh: erst leeres Repo auf github.com anlegen, dann:
git remote add origin https://github.com/DEIN-USER/pitboss.git
