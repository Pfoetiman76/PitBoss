...
git branch -M main
git add -A
git commit -m "PitBoss 0.4.7.5" || true
git rev-parse HEAD >/dev/null 2>&1 || { echo "[FEHLER] Kein Commit - git-Identitaet pruefen."; exit 1; }
gh repo view Pfoetiman76/pitboss >/dev/null 2>&1 || gh repo create pitboss --public --disable-wiki
git remote get-url origin >/dev/null 2>&1 || git remote add origin https://github.com/Pfoetiman76/pitboss.git
git push -u origin main
git tag v0.4.7.5 2>/dev/null || true
git push origin v0.4.7.5
echo "Fertig. Actions: https://github.com/Pfoetiman76/pitboss/actions"
