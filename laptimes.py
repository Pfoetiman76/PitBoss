"""
PitBoss — Standard-Rundenzeiten-Referenz  (additiv, 0.4.8.3)
=========================================================================
...
"LMGT3": {
 ...
 "Barcelona":"1:41.05","Daytona":"1:45.10","Laguna Seca":"1:21.86","Circuit de la Sarthe":"3:56.29","Circuit de la Sarthe (straight)":"3:40.43",
...
"LMH": {
 ...
 "Barcelona":"1:28.97","Daytona":"1:32.93","Laguna Seca":"1:12.72","Circuit de la Sarthe":"3:24.22","Circuit de la Sarthe (straight)":"3:10.72",
...
"LMP3": {
 ...
 "Barcelona":"1:35.24","Daytona":"1:40.55","Laguna Seca":"1:17.80","Circuit de la Sarthe":"3:45.20","Circuit de la Sarthe (straight)":"3:30.74",
...
"LMP2 ELMS": {
 ...
 "Barcelona":"1:27.96","Daytona":"1:33.92","Laguna Seca":"1:12.13","Circuit de la Sarthe":"3:27.86","Circuit de la Sarthe (straight)":"3:13.67",
...
"LMP2 WEC": {
 ...
 "Barcelona":"1:31.85","Daytona":"1:37.93","Laguna Seca":"1:14.31","Circuit de la Sarthe":"3:34.58","Circuit de la Sarthe (straight)":"3:19.42",
...
"GTE": {
 ...
 "Barcelona":"1:40.21","Daytona":"1:43.50","Laguna Seca":"1:19.97","Circuit de la Sarthe":"3:51.54","Circuit de la Sarthe (straight)":"3:37.37",
...
# 0.5.7.0: Live-Streckennamen (LMU mTrackName) tolerant auf Referenz-Schluessel abbilden.
# LMU-Schreibweise -> Referenz-Schluessel (lowercase-Schluessel).
TRACK_ALIASES = {
    "le mans": "Circuit de la Sarthe", "circuit de la sarthe": "Circuit de la Sarthe", "sarthe": "Circuit de la Sarthe",
    "spa": "Spa", "spa francorchamps": "Spa", "circuit de spa francorchamps": "Spa",
    "monza": "Monza", "autodromo nazionale monza": "Monza",
    "interlagos": "Interlagos", "sao paulo": "Interlagos", "autodromo jose carlos pace": "Interlagos",
    "imola": "Imola", "autodromo enzo e dino ferrari": "Imola",
    "fuji": "Fuji (classic)", "fuji speedway": "Fuji (classic)",
    "portimao": "Portimao", "algarve": "Portimao", "autodromo internacional do algarve": "Portimao",
    "qatar": "Qatar", "lusail": "Qatar", "losail": "Qatar", "lusail international circuit": "Qatar",
    "bahrain": "Bahrain (wec)", "bahrain international circuit": "Bahrain (wec)",
    "cota": "COTA", "circuit of the americas": "COTA", "austin": "COTA",
    "silverstone": "Silverstone (GP)", "silverstone grand prix": "Silverstone (GP)",
    "sebring": "Sebring", "sebring international raceway": "Sebring",
    "barcelona": "Barcelona", "catalunya": "Barcelona", "circuit de barcelona catalunya": "Barcelona",
    "paul ricard": "Paul Ricard", "le castellet": "Paul Ricard",
    "daytona": "Daytona", "daytona international speedway": "Daytona", "daytona international speedway road course": "Daytona",
    "laguna seca": "Laguna Seca", "weathertech raceway laguna seca": "Laguna Seca",
}


def _norm(x: str) -> str:
    x = unicodedata.normalize("NFKD", (x or "").lower())
    x = "".join(c for c in x if not unicodedata.combining(c))  # Akzente entfernen
    return re.sub(r"[^a-z0-9]+", " ", x).strip()


def resolve_track(live_name: str, cls: str | None = None):
    """Ordnet einen Live-Streckennamen einem Referenz-Schluessel zu oder gibt None
    zurueck (dann ist fuer diese - evtl. neue - Strecke keine Referenz hinterlegt)."""
    if not live_name:
        return None
    keys = list(ALIEN.get(cls, {}).keys()) if cls in ALIEN else list(ALIEN.get("LMGT3", {}).keys())
    ln = _norm(live_name)
    for k in keys:                                  # 1) exakter (normalisierter) Treffer
        if _norm(k) == ln:
            return k
    if ln in TRACK_ALIASES and TRACK_ALIASES[ln] in keys:   # 2) Alias exakt
        return TRACK_ALIASES[ln]
    for alias, key in TRACK_ALIASES.items():        # 3) Alias als Praefix/Teilstring
        if (ln == alias or ln.startswith(alias + " ") or alias in ln) and key in keys:
            return key
    for k in keys:                                  # 4) Basisname (vor der Layout-Klammer)
        base = _norm(k.split("(")[0])
        if base and (base == ln or base in ln or ln in base):
            return k
    return None
